            ...
            @foreach($produks as $p)
                <tr>
                    ...
                    <td>
                        ...
                        <button class="btn btn-info" data-bs-toggle="modal" data-bs-target="#addGambar{{ $p->id }}">
                            Tambah gambar
                        </button>
                    </td>
                </tr>
    ...
    <div class="modal-content">
      ...
      <form action="{{ route('produk.update') }}" method="POST">
        ...
        <div class="modal-body">
            ...
            <div class="mb-3">
                <label>Deskripsi Produk</label>
                <textarea name="deskripsi_produk" class="form-control" rows="3" required>{{ $p->deskripsi_produk }}</textarea>
            </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button>
...
{{-- ================= MODAL ADD GAMBAR PER ITEM ================= --}}
@foreach($produks as $p)
<div class="modal fade" id="addGambar{{ $p->id }}" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <form action="{{ route('gambar.tambah', $p->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="modal-header">
